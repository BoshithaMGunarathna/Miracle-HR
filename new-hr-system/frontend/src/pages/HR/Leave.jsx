...
const [startDate, setStartDate] = useState(null);
  ...
  //       setAlertMessage("Error fetching leave data");
    ...
    axiosClient.get('/hr/leave')
      .then(response => {
        if (response.data.status === "error") {
          ...
          setAlertMessage("No Leave Data Found");
        setAlertSeverity('error');
        setShowAlert(true);
        setTimeout(() => {
          setShowAlert(false);
        ...
        setLoading(false);
          setLeaveHistory([]); // Set leaveHistory to an empty array
        } else {
  ...
  const filteredData = leaveHistory.filter(item => {
    ...
    const matchesDate =
    (startDate && endDate)
      ? firstDay <= endDate && lastDay >= startDate // Check if the leave period overlaps with the selected range
      : true;
