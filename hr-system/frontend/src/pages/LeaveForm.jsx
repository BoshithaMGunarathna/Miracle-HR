...
useEffect(() => {
  ...
  axios.get(`http://localhost:8081/leave/${emp_id}`)
    .then(response => {
      if (response.data.status === "success") {
        ...
        console.log('Fetched Data:', leave[0].annual_leave_count);
    ...
    if (startDate && endDate) {
      ...
      if (reason==='annual'){      
        if (annualLeaveCount !== null && (annualLeaveTaken + days) > annualLeaveCount) {
          ...
          alert('You do not have enough Annual Leaves available.');
          isValid = false;
        }
      }
      ...
      if (reason==='casual'){
        if (casualLeaveCount !== null && (casualLeaveTaken + days) > casualLeaveCount) {
          ...
          alert('You do not have enough Casual Leaves available.');
          isValid = false;
        }
      }
    }
    ...
    if (isValid) {
      ...
      if (updatedRowData){
          if(updatedRowData.leave_id){
            ...
            axios.post("http://localhost:8081/update-leave", formData)
            .then(response => {
              if (response.data.status === "success") {
                alert("Leave update application submitted successfully!");
            ...
            .catch(error => {
              ...
              alert("Leave update submission failed. Please try again.");
            });
          }
          if(updatedRowData.leave_request_id){
            ...
            axios.put(`http://localhost:8081/update-leave/${updatedRowData.leave_request_id}`, formData)
            .then(response => {
              if (response.data.status === "success") {
            ...
            .catch(error => {
              console.error("There was an error with the submission:", error);
              alert("Leave update failed. Please try again.");
            });

          }
      }else{
        axios.post("http://localhost:8081/leave", formData)
        .then(response => {
          if (response.data.status === "success") {
  ...
  <div className="mt-4">
    ...
    >
      ...
      <option
        ...
        disabled={
          !annualLeaveCount || 
          annualLeaveCount <= annualLeaveTaken || annualLeaveCount <= annualLeaveTaken  ||annualLeaveCount === null
        }
      >
        Annual Leave
      </option>
